...
import Ent from "../../componentes/Entradas";
...
const Entradas = ({ navigation }) => {
  ...
  const BuscarEntradas = async () => {
    if (!validarFiltro) {
      ...
      await Axios.get("/entradas/buscarnombreRol?nombreRol=" + Filtro + "%") //BUSCA EL LOGIN DEL USUARIO
        .then(async (data) => {
  ...
  const BuscarEntradasTodos = async () => {
    var textoMensaje = "";
    setEspera(true);
    Axios.defaults.headers.common["Authorization"] = "Bearer " + token;
    await Axios.get("/entradas/listar")
      .then(async (data) => {
        const json = data.data;
        ...
        {espera ? (
          ...
          <>
            ...
            <View style={Estilos.contenedorControles}>
              <FlatList
                data={lista}
                renderItem={({ item }) => <Ent entrada={item}></Ent>}
