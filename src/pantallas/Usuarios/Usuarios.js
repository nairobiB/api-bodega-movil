...
const Roles = ({ navigation }) => {
  ...
  const BuscarUsuarios = async () => {
    if (!validarFiltro) {
      ...
      await Axios.get("/users/buscarnombreUsuario?usuario=" + Filtro + "%") //BUSCA EL LOGIN DEL USUARIO
        .then(async (data) => {
        ...
        {espera ? (
          ...
          <>
            ...
            <View style={Estilos.contenedorControles}>
              <FlatList
                data={lista}
                renderItem={({ item }) => <Usuario usuario={item}></Usuario>}
